    ...
    T: Indexer + Send + Sync + Clone,
{
    ...
    fn update_indexes(&self) -> io::Result<()> {
        ...
        // If we have an indexer
        if let Some(indexer) = &self.indexer {
            // We want to find any unindexed blobs and pass them to the indexer

            // TODO: Don't always pass all blobs
...
mod tests {
    ...
    #[test]
    fn basic() {
        let (mut storage, _tempdir_guard) = new_storage::<TestIndexer>();
    ...
    fn test_update_indexes() {
        #[derive(Clone)]
        struct MyTestIndexer;

        impl Indexer for MyTestIndexer {
            fn run(&self, unindexed_blobs: Vec<Vec<u8>>) -> Result<Vec<UniqueIndexUpdate>, ()> {
                assert_eq!(&unindexed_blobs, &vec![b"foo".to_vec(), b"bar".to_vec()]);

                Ok(unindexed_blobs
                    .into_iter()
                    .map(|blob| UniqueIndexUpdate {
                        index_name: "an_index".to_string(),
                        unique_key: String::from_utf8(blob).expect("invalid utf8"),
                    })
                    .collect())
            }
        }

        let (mut storage, _tempdir_guard) = new_storage::<MyTestIndexer>();
        storage.set_indexer(MyTestIndexer {});
    ...
    fn new_storage<T: 'static + Indexer + Send + Sync + Clone>() -> (Storage<T>, tempfile::TempDir)
    {
        let temp = tempdir();
